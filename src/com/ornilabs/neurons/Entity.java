...
public class Entity {
	...
	public Entity(IRobot managedRobot) {
		brain = new MultiLayerPerceptron(1, 3, 3, 1, 0.01);
	...
	public double[] calculateAndTrain(IRobot target) {
		double[] targetCoords = target.getPosition();
...
//				double theta = Math.atan((robotCible.getPosition()[0]-robot.getPosition()[0])/(robotCible.getPosition()[1]-robot.getPosition()[1]));
		...
		double distance = Math.sqrt(Launch.squareDistance(managedRobot.getPosition(), target.getPosition()));
...
//		System.out.println("@"+outputExpected[1]);
	...
	public void update(IRobot target) {
